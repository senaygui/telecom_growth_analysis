...
class MockConnection:
    ...
    def fetchall(self):
        ...
        return [(1, 'Senay', 25), (2, 'Tekle', None), (3, 'BBB', 30)]
...
def test_sql_to_dataframe():
    ...
    assert df.values.tolist() == [[1, 'Senay', 25], [2, 'Tekle', None], [3, 'BBB', 30]]


# Test find_missing_values function
def test_find_missing_values():

    df = TestDataPipline({'ID': [1, 2, 3], 'Name': ['Senay', 'Tekle', 'BBB'], 'Age': [25, None, 30]})
...
def test_handle_missing_values():
    ...
    df = TestDataPipline({'ID': [1, 2, 3], 'Name': ['Senay', 'Tekle', None], 'Age': [25, None, 30]})

    df_filled = handle_missing_values(df)

    assert isinstance(df_filled, pd.DataFrame)
    assert df_filled.columns.tolist() == ['ID', 'Name', 'Age']
    assert df_filled.values.tolist() == [[1, 'Senay', 25], [2, 'Tekle', 27.5], [3, 'N/A', 30]]
